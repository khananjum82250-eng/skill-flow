...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const profileUsernameStatus = document.getElementById('profileUsernameStatus');
    const defaultAvatarUrl = avatarOptions[0]?.dataset.url || '';
    let savedAvatarUrl = selectedAvatarInput?.value || defaultAvatarUrl;
    let savedUsername = profileUsername?.value || '';
    ...
    function setProfileUsernameStatus(message, state = '') {
        if (!profileUsernameStatus) return;
        profileUsernameStatus.textContent = message;
        profileUsernameStatus.classList.toggle('is-valid', state === 'valid');
        profileUsernameStatus.classList.toggle('is-invalid', state === 'invalid');
    }
    ...
    function submitProfileData() {
        ...
        const usernameError = validateUsernameFormat(payload.username);
        if (usernameError) {
            setProfileUsernameStatus(usernameError, 'invalid');
        ...
        .then((data) => {
            if (data.success) {
                ...
                if (profileUsername && data.username) {
                    ...
                    setProfileUsernameStatus('Username saved.', 'valid');
                }
                return true;
            }
    ...
    let profileUsernameTimer;
    let latestProfileUsernameCheck = '';
    profileUsername?.addEventListener('input', () => {
        const username = normalizeUsername(profileUsername.value);
        profileUsername.value = username;
        window.clearTimeout(profileUsernameTimer);

        const formatError = validateUsernameFormat(username);
        if (formatError) {
            setProfileUsernameStatus(formatError, 'invalid');
            return;
        }
        if (username === savedUsername) {
            setProfileUsernameStatus('Current username.', '');
            return;
        }

        setProfileUsernameStatus('Checking username...', '');
        profileUsernameTimer = window.setTimeout(() => {
            latestProfileUsernameCheck = username;
            fetch(`/api/username/check?username=${encodeURIComponent(username)}&current=1`)
                .then((res) => res.json())
                .then((data) => {
                    if (latestProfileUsernameCheck !== username) return;
                    if (data.available) setProfileUsernameStatus('Username is available.', 'valid');
                    else setProfileUsernameStatus(data.error || 'Username already taken.', 'invalid');
                })
                .catch(() => setProfileUsernameStatus('Unable to check username right now.', 'invalid'));
        }, 300);
    });
    ...
    if (btnDeleteAccount) {
        btnDeleteAccount.addEventListener('click', async () => {
            const confirmed = await SkillFlowUI.confirmDialog('Are you sure you want to delete your account? This action cannot be undone.', {
